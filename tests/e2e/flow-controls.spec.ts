...
test("domestic filter and table search are wired to dashboard-data requests", async ({ page }) => {
  ...
  await expect
    .poll(() =>
      requestUrls.some(
        (url) => url.searchParams.get("view") === "globe_flows" && url.searchParams.get("includeDomestic") === "true"
      )
    )
    .toBe(true);
...
test("country focus search pins state, drag does not clear, and hover does not refetch", async ({ page }) => {
  const requestUrls: URL[] = [];
  page.on("request", (request) => {
    const url = new URL(request.url());
    if (!url.pathname.endsWith("/api/v1/dashboard-data")) return;
...
test("country with no corridors shows empty state and unsupported filters are informational", async ({ page }) => {
  ...
  await expect(page.getByText(/not applied to the globe/i)).toBeVisible();

  await page.getByLabel("Search country").fill("Antarctica");
  await page.getByRole("button", { name: /^Focus$/i }).click();
  await expect(page.getByText(/No donation corridors found for this country under the current filters/i)).toBeVisible();
});
